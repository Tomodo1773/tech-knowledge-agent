...
MVPでは個人利用・dev一環境を前提とする。自動fallbackやrollback機構は作らず、Queue滞留・poison message、Function/Agentエラー、同期停止、予算超過を必要に応じて確認する。監視項目の詳細は[telemetry.md](telemetry.md)を参照する。
...
rootの`postdeploy` hookは全serviceのdeploy後に一度だけ動き、**Agent identityへのrole付与だけを行う。** Responses endpointをFunction Appのapp設定へ書き戻す段は無い。Workerは`FOUNDRY_PROJECT_ENDPOINT`とagent名からSDKにendpointを組み立てさせるため、deploy後にしか決まらない値がそもそも無い([architecture.md](architecture.md#hosted-agentの呼び出し))。

hookが残っているのは、role assignmentがdeploy後にしか作れないためである。公式も「role assignmentはagent identityに対して必要なので、agent作成後にしか作れない」と明記しており、Bicepへ移す道は無い([Hosted agent permissions reference](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/hosted-agent-permissions))。Hosted Agentのidentityは Microsoft Entra の Agent ID としてFoundryが自動払い出しするもので、user-assigned managed identityを持ち込むこともできない。

hookは`azd ai agent show knowledge-agent --output json`の`instance_identity.principal_id`をUUIDとして取得し、subscription、resource group、`COSMOS_ACCOUNT_NAME`、固定scope `/dbs/knowledge/colls/chunks`を明示してCosmos Built-in Data Readerを冪等作成する。あわせてFoundry accountへ`Cognitive Services OpenAI User`、Application Insightsへ`Monitoring Metrics Publisher`を付ける。beta.9のhookは`AGENT_KNOWLEDGE_AGENT_INSTANCE_IDENTITY_PRINCIPAL_ID`のようなidentity専用の環境変数をhookへ注入しない(実際に注入されるのは`ENDPOINT` / `NAME` / `PROJECT_ENDPOINT` / `VERSION` / `RESPONSES_ENDPOINT`のみ)ため、`azd ai agent show`から都度取得する。Azure CLIとrole scriptのstdout / stderrは内部で破棄し、成功・失敗ともprincipal、scope、resource名を出さず固定文言だけを返す。値の欠落・不正、またはCLIが失敗した場合はhook全体を失敗させる。
